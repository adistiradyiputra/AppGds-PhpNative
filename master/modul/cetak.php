...
$query=mysqli_query($koneksi,"SELECT tp.id, s.nama, s.kelas, p.pelanggaran, p.point, tp.note, tp.tanggal
										FROM t_pelanggaran AS tp
										LEFT JOIN siswa AS s ON s.id = tp.id_siswa
										LEFT JOIN pelanggaran AS p ON p.id = tp.id_pelanggaran WHERE s.id='$id'");
...
$pdf->Cell(8.2, 0.8, 'Total', 1, 0, 'L');
$pdf->Cell(2, 0.8, $total, 1, 1, 'C');
$pdf->Cell(8.2, 0.8, 'Status', 1, 0, 'L');
